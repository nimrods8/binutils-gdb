...
#ifndef TUI_TUI_CONSOLE_H
#define TUI_TUI_CONSOLE_H
...
struct tui_console_item_window
{
  tui_console_item_window () = default;

  DISABLE_COPY_AND_ASSIGN (tui_console_item_window);

  tui_console_item_window (tui_console_item_window &&) = default;
...
struct tui_console_window : public tui_win_info
{
  tui_console_window () = default;

  DISABLE_COPY_AND_ASSIGN (tui_console_window);
...
private:
  ...
  /* Display the registers starting at line line_no in the data
     window.  Answers the line number that the display actually
     started from.  If nothing is displayed (-1) is returned.  */
  void display_console_from( int line_no);
  ...
  std::vector<tui_console_item_window> m_console_content;
