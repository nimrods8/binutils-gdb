/* TUI display memory data in window.
...
#ifndef TUI_TUI_MEMDUMP_H
#define TUI_TUI_MEMDUMP_H
...
struct tui_memdump_item_window
{
  tui_memdump_item_window () = default;

  DISABLE_COPY_AND_ASSIGN (tui_memdump_item_window);

  tui_memdump_item_window (tui_memdump_item_window &&) = default;
...
struct tui_memdump_window : public tui_win_info
{
...
private:
  ...
  /* Display the registers starting at line line_no in the data
     window.  Answers the line number that the display actually
     started from.  If nothing is displayed (-1) is returned.  */
  void display_memdump_from( int line_no);
  ...
  std::vector<tui_memdump_item_window> m_memdump_content;
...
void initialize_tui_memdump( void);


#endif /* TUI_TUI_MEMDUMP_H */
